...
public class SkillRunner : MonoBehaviour
{
    ...
    public void UseSkill(int slotIdx , CoolDownView cooldown)
    {
        ...
        {
            Debug.LogError("SkillRunner arrays are null");
            return;
        }

        if (slotIdx < 0 || slotIdx >= _skills.Length || slotIdx >= _cooldowns.Length)
        {
            Debug.LogError("UseSkill slotIdx out of range: " + slotIdx
                + " skillsLen=" + _skills.Length
                + " cooldownsLen=" + _cooldowns.Length);
            return;
        }
